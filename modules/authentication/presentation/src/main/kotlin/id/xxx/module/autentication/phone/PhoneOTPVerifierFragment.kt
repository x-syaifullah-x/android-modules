...
class PhoneOTPVerifierFragment : Fragment() {
    ...
    private val onBackPressedCallback = object : OnBackPressedCallback(true) {
        override fun handleOnBackPressed() {
            ...
            sfm.beginTransaction().replace(id, PhoneFragment::class.java, null).commit()
        }
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        vBinding.buttonVerify.setOnClickListener {
            if (vBinding.progressBar.visibility != View.VISIBLE) {
                ...
                viewLifecycleOwner.lifecycleScope.launch {
                    ...
                    val type = AuthenticationType.Phone(verificationId = verificationId, code = code)
                    val res = iSign()?.onAuthentication(type)
                    if (res is IAuthenticationResult.Error) {
                        Toast.makeText(
                            context?.applicationContext, res.err.message, Toast.LENGTH_LONG
                        ).show()
                    }
